/**
 * Player headshot enrichment. football-data.org gives real players + stats but no photos, so we
 * resolve each player's Wikipedia thumbnail by name and store it on `photoUrl`. The frontend's
 * PlayerAvatar prefers a real photo and falls back to a generated avatar when one is missing.
 *
 * Used by both the one-off `enrich:photos` script and the server's background boot enrichment.
 * Idempotent: by default only players without a photo are resolved.
 */
...
export async function photoForName(name: string): Promise<string> {
...
/**
 * Backfill headshots. When `force` is false (default) only players missing a photo are resolved,
 * so it's cheap to re-run on every boot. `onMissing` reports names with no match (script logging).
 */
export async function enrichMissingPhotos(
  opts: { force?: boolean; onMissing?: (name: string) => void } = {},
): Promise<EnrichResult> {
  const { force = false, onMissing } = opts;
  ...
  return { scanned: players.length, filled };
}
